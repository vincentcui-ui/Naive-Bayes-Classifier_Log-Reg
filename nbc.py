...
class NaiveBayes:
    ...
    def compute_class_probabilities(self, y_train: np.ndarray) -> Dict[Any, float]:
        ...
        # >>> YOUR CODE HERE >>>
        total_samples = y_train.shape[0] + len(classes) * self.alpha
        class_probabilities = {c: (count + self.alpha) / total_samples for c, count in zip(classes, counts)}
    ...
    def compute_feature_probabilities(self, X_j_train: np.ndarray, y_train: np.ndarray) -> Dict[Tuple[Any, Any], float]:
        ...
        for c in class_labels:
            ...
            feature_values_for_class = X_j_train[indices]
            feature_counts = dict(zip(*np.unique(feature_values_for_class, return_counts=True)))
            total_samples_for_class = len(indices)

            for feature_value in unique_features:
                count = feature_counts.get(feature_value, 0)
                probability = (count + self.alpha) / (total_samples_for_class + self.alpha * len(unique_features))
                feature_probabilities[(feature_value, c)] = probability
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray) -> None:
        ...
        for j in range(n_features):
            self.feature_probabilities[j] = self.compute_feature_probabilities(X_train[:, j], y_train)
        # <<< END OF YOUR CODE <<<
            ...
            for c, label in enumerate(self.class_labels):
                ...
                for j in range(self.n_features):
                    ...
                    if (X_test[i, j], label) not in self.feature_probabilities[j]:
                        # >>> YOUR CODE HERE >>>
                        prob *= self.feature_probabilities[j].get(('NA', label), 1) 
                        # <<< END OF YOUR CODE <<<
                    else:
                        # If the feature value is in the training data, use the
                        # probability of the feature value
                        # >>> YOUR CODE HERE >>>
                        prob *= self.feature_probabilities[j][(X_test[i, j], label)]
                        # <<< END OF YOUR CODE <<<
    ...
    def predict(self, probabilities: np.ndarray) -> np.ndarray:
        ...
        y_pred = self.class_labels[max_prob_indices]
        # <<< END OF YOUR CODE <<<
        return y_pred
    ...
    def evaluate(self, y_test: np.ndarray, probabilities: np.ndarray) -> Tuple[float, float]:
        ...
        for i, true_label in enumerate(y_test):
            prob_true_label = probabilities[i, np.where(self.class_labels == true_label)[0][0]]
            squared_diff = (1 - prob_true_label) ** 2
            squared_diffs.append(squared_diff)
        squared_loss = np.mean(squared_diffs)
        # <<< END OF YOUR CODE <<<
